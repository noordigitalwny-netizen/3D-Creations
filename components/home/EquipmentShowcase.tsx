...
import { Scan, Printer, Zap, ArrowRight } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {machines.map((m, idx) => (
            <div
              key={idx}
              className="rounded-lg bg-white border border-gray-200 p-6 flex flex-col justify-between shadow-sm hover:shadow-md transition-shadow"
            >
              <div>
                ...
                {/* Specs List */}
                <div className="bg-gray-50 rounded-md p-3.5 space-y-2 mb-6 border border-gray-200">
                  {m.specs.map((s, i) => (
                    <div key={i} className="flex justify-between text-xs">
                      <span className="text-slate-500">{s.label}:</span>
                      <strong className="text-slate-900 font-mono">{s.val}</strong>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <Link
                href={m.link}
                className="w-full inline-flex items-center justify-center space-x-2 py-2.5 px-4 rounded-md text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 transition-colors"
              >
